...
router.get('/unlocks', protect, async (req, res, next) => {
    try {
        ...
        // Công tắc tổng tắt → trả danh sách RỖNG. lockInfo() bên client coi key
        // không có trong danh sách là "không khoá", nên mọi ổ khoá biến mất mà
        // không phải sửa một dòng frontend nào. unlockedBetween() cũng thành
        // rỗng → lên cấp không hiện popup "vừa mở khoá X", đúng vì chẳng có gì
        // để mở.
        if (!locking) {
