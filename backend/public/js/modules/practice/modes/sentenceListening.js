// ===================================
// NGHE CHUỖI TỪ - MULTI-WORD LISTENING
// Nghe 3 từ đọc liên tiếp → tìm và chọn đúng 3 từ trong lưới 8 từ
// Luyện trí nhớ thính giác & phân biệt từ trong luồng nghe - TOEIC 850+
// ===================================

const SentenceListening = {
    ...
    /**
     * Generate questions - mỗi câu hỏi: 3 từ đích + 5 từ nhiễu = lưới 8 từ
     */
    async generateQuestions() {
        ...
        for (let i = 0; i < limit; i++) {
            ...
            // 5 distractors from remaining words
            const distractors = shuffled
                .filter(w => !targetWords.find(t => t.en === w.en))
                .sort(() => Math.random() - 0.5)
                .slice(0, 5);

            // Grid of 8 words shuffled
    ...
    showQuestion() {
        ...
        // Auto play sequence after short delay
    ...
    /**
     * Play 3 words in sequence, waiting for each to finish before playing next
     */
    playSequence(words) {
        const btn = document.getElementById('sl-play-btn');
        if (btn) btn.classList.add('playing');

        const gap = 400; // pause between words after audio ends
    ...
    toggleWord(wordEn, btn) {
        if (this.selectedWords.has(wordEn)) {
            ...
            if (this.selectedWords.size >= 3) return; // Max 3 selections
    ...
    submitAnswer(question) {
        ...
        // Visual feedback on each grid button
        document.querySelectorAll('.sl-grid-word-btn').forEach(btn => {
    ...
    setupHintSkipListeners() {
        this._hintHandler = () => {
            ...
            this.hintUsed = true;
            // Find a target word not yet selected and highlight it
...
// Make global
window.SentenceListening = SentenceListening;

if (typeof module !== 'undefined' && module.exports) {
    module.exports = SentenceListening;
}
