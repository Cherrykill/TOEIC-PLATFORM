...
function openUserFormModal(title, mode, userData = {}) {
    ...
    if (isEditMode) {
        passwordLabel.innerHTML = 'Mat khau <small>(de trong neu khong doi)</small>';
...
function displayUsers() {
    ...
    if (!paginatedUsers || !paginatedUsers.length) {
        tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:30px;">Khong tim thay tai khoan nao</td></tr>';
...
function attachUserListeners() {
    ...
    document.querySelectorAll(".btn-user-togglelock").forEach((btn) => {
        btn.addEventListener("click", async () => {
            ...
            if (!confirm(`Ban co chac muon ${action} tai khoan "${username}"?`)) return;
    ...
    document.querySelectorAll(".btn-user-delete").forEach((btn) => {
        btn.addEventListener("click", async () => {
            const id = btn.dataset.id;
            const username = btn.dataset.username;
            if (!confirm(`Ban co chac chan muon XOA tai khoan "${username}" (ID: ${id})?`)) return;
    ...
    try {
        ...
        if (data.success) {
            ...
            setTimeout(async () => {
                ...
                alert(id ? "Thong tin tai khoan da duoc luu thanh cong!" : "Da them tai khoan moi thanh cong!");
            }, 200);
        } else {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("btn-add-user")?.addEventListener("click", () => openUserFormModal("Them tai khoan moi", "add"));
    ...
    document.getElementById("btn-submit-json")?.addEventListener("click", () => { if (typeof submitJsonImport === 'function') submitJsonImport(); });

    document.getElementById("tab-manual")?.addEventListener("click", () => { if (typeof switchWordModalTab === 'function') switchWordModalTab('manual'); });
    document.getElementById("tab-json")?.addEventListener("click",   () => { if (typeof switchWordModalTab === 'function') switchWordModalTab('json'); });
    ...
    document.getElementById("btn-ai-fill")?.addEventListener("click", async () => {
        ...
        if (!word) { showToast('Vui long nhap tu tieng Anh truoc!', 'warning'); wordInput?.focus(); return; }
    ...
    document.getElementById("add-word-form")?.addEventListener("submit", async (e) => {
        ...
        if (!wordData.en || !wordData.vn || !wordData.part || !wordData.type) {
            showToast('Vui long dien day du thong tin bat buoc', 'error'); return;
        }
        try {
            let res, data;
            if (editMode) {
            ...
            if (data.success) {
                ...
                document.getElementById("add-word-form").reset();
                delete modal.dataset.editMode; delete modal.dataset.originalEn;
                modal.querySelector("h3").innerHTML = '+ Add New Word';
                modal.querySelector('button[type="submit"]').textContent = 'Add Word';
                setTimeout(async () => {
                    if (editMode) { await loadVocabulary(vocabCurrentPage, vocabCurrentPart); }
                    else { vocabCurrentPage = 1; vocabCurrentPart = ''; await loadVocabulary(1, ''); }
                    await loadVocabularyStats();
                    await loadRecentActivities();
                    alert(editMode ? "Tu vung da duoc cap nhat thanh cong!" : "Tu vung da duoc them thanh cong!");
                }, 200);
            } else {
                alert("Loi: " + (data.message || `Khong the ${editMode ? 'cap nhat' : 'them'} tu vung.`));
            }
        } catch (error) {
            alert(`Loi ket noi: Khong the ${editMode ? 'cap nhat' : 'them'} tu vung.`);
        }
    });
    ...
    document.getElementById("action-btn-export-vocab")?.addEventListener("click", async () => {
        ...
        } catch (error) { alert("Loi khi xuat file: " + error.message); }
    });
    ...
    document.getElementById("action-btn-backup-db")?.addEventListener("click", async () => {
        try {
            ...
            const backup = { timestamp: new Date().toISOString(), vocabulary: vocabData.data || [], users: usersData.data || [] };
        ...
        } catch (error) { alert("Loi khi backup: " + error.message); }
    });
    ...
    document.getElementById("action-btn-reload-cache")?.addEventListener("click", async () => {
        if (confirm("Ban co chac muon tai lai du lieu tu file?")) {
            try {
                ...
                loadDashboard();
            } catch (error) { alert("Loi khi reload cache: " + error.message); }
        }
    });
    ...
    document.getElementById("btn-quick-delete-close")?.addEventListener("click", () => stopQuickDelete());
});
