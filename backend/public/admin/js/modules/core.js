...
async function updateLocalPartFilter(vocabData) {
    try {
        ...
        if (data && data.success && Array.isArray(data.data)) {
            ...
            renderPartFilter(data.data.map(part => ({ part, count: counts[part] })));
            return;
        }
        ...
        const localSource = vocabData || window._lastVocabData || localVocabularyData || [];
        const parts = [...new Set(localSource.map(w => w.part).filter(Boolean))].sort();
        renderPartFilter(parts.map(part => ({
            part,
            count: (localVocabularyData || []).filter(w => w.part === part).length || undefined,
        })));
    }
}
...
async function initDashboard() {
    ...
    // Topics tab buttons
    document.getElementById('btn-add-topic')?.addEventListener('click', () => showTopicModal());
